...
#
UPSCABLE {{ apcupsd_conf.get('upscable', 'usb') }}
...
#
UPSTYPE {{ apcupsd_conf.get('upstype', 'usb') }}
DEVICE {{ apcupsd_conf.get('device', '') }}
...
ONBATTERYDELAY {{ apcupsd_conf.get('onbatterydelay', '10') }}
...
BATTERYLEVEL {{ apcupsd_conf.get('batterylevel', '5') }}

# If during a power failure, the remaining runtime in minutes 
# (as calculated internally by the UPS) is below or equal to MINUTES,
# apcupsd, will initiate a system shutdown.
MINUTES {{ apcupsd_conf.get('minutes', '2') }}
...
ANNOY {{ apcupsd_conf.get('annoy', '60') }}

# Initial delay after power failure before warning users to get
# off the system.
ANNOYDELAY {{ apcupsd_conf.get('annoydelay', '60') }}

# The condition which determines when users are prevented from
# logging in during a power failure.
# NOLOGON <string> [ disable | timeout | percent | minutes | always ]
NOLOGON {{ apcupsd_conf.get('nologon', 'disable') }}
...
#   card.
UPSCLASS {{ apcupsd_conf.get('upsclass', 'standalone') }}

# UPSMODE [ disable | share ]
#   Normally disable unless you share a UPS using an APC ShareUPS card.
UPSMODE {{ apcupsd_conf.get('upsmode', 'disable') }}
...
DATATIME {{ apcupsd_conf.get('datatime', '0') }}
...
SENSITIVITY {{ apcupsd_conf.get('sensitivity', 'H') }}

# UPS delay after power return (seconds)
# WAKEUP 000 060 180 300   (default = 0)
WAKEUP {{ apcupsd_conf.get('wakeup', '60') }}

# UPS Grace period after request to power off (seconds)
# SLEEP 020 180 300 600    (default = 20)
SLEEP {{ apcupsd_conf.get('sleep', '120') }}
...
LOTRANSFER {{ apcupsd_conf.get('lotransfer', '208') }}
...
HITRANSFER {{ apcupsd_conf.get('hitransfer', '253') }}

# Battery charge needed to restore power
# RETURNCHARGE 00 15 50 90 (default = 15)
RETURNCHARGE {{ apcupsd_conf.get('returncharge', '15') }}

# Alarm delay 
# 0 = zero delay after pwr fail, T = power fail + 30 sec, L = low battery, N = never
# BEEPSTATE 0 T L N        (default = 0)
BEEPSTATE {{ apcupsd_conf.get('beepstate', 'T') }}

# Low battery warning delay in minutes
# LOWBATT 02 05 07 10      (default = 02)
LOWBATT {{ apcupsd_conf.get('lowbatt', '02') }}
...
OUTPUTVOLTS {{ apcupsd_conf.get('outputvolts', '230') }}

# Self test interval in hours 336=2 weeks, 168=1 week, ON=at power on
# SELFTEST 336 168 ON OFF  (default = 336)
SELFTEST {{ apcupsd_conf.get('selftest', '168') }}
