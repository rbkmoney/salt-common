...
class influxdb09(influxdb):
    ...
    def format_metric(self, timestamp, path, tags, value, customer, warn, crit, var_min, var_max):
        if not self.influxdb_line_protocol:
            ...
            return {
                    ...
                    "measurement": path,
                    "tags": tags,
                    "fields": {"value": value, "warn":warn, "crit": crit, "min": var_min, "max": var_max}}
        return '%s,%s warn=%s,crit=%s,min=%s,max=%s,value=%s %d' % (
        #return '%s,%s warn=%s,crit=%s,min=%s,max=%s,value=%s %d' % (
                path,
    ...
    def send(self, metrics):
        ...
        for m in metrics:
            ...
            try:
               servicestatetype = m.SERVICESTATETYPE
            except ValueError:
               servicestatetype = "\ "
           ...
           #    customer = "\ "
            ...
            tags = {"customer": customer, "check": m.LABEL, "host": m.HOSTNAME, "displayname": displayname, "uom": uom, "hoststate": hoststate, "hoststatetype": hoststatetype, "servicestate": servicestate, "servicestatetype": servicestatetype}

            tags.update(self.influxdb_extra_tags)

            perfdata.append(self.format_metric(int(m.TIMET), path,
                            tags, value, customer, warn, crit, var_min, var_max))
